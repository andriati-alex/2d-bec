...
def HarmonicTrap(x, y, n):
    ...
    for i in range(int(n)):
        ...
        expargy = 1.0j * ky[i] * y - ((y - midy) / sigy)**2;
...
S = HarmonicTrap(x,y,5).reshape(1,nx*ny)[0];

folder = str(Path.home()) + '/AndriatiLibrary/2d-bec/input/';

np.savetxt(folder + 'trapped_init.dat', S.T, fmt='%.15E');

f = open(folder + 'trapped_domain.dat', "w");
